...
//Creacion html
const ulElement = document.querySelector('#peliculas');
const searchElement = document.querySelector('#buscarPelicula');
let movies = [];
...
const main = () => {
    fetch('./api/characters.json')
        ...
        .then(movies => movies.forEach(renderCardContact))
        // .then(movies => {
        //     let i = 0;
        //     while(i < movies.length) {
        //         renderCardContact(movies[i]);
        //         i++;
        //     }
        // })
};
...
const renderCardContact = (element) => {
    ...
    cardDiv.addEventListener('click', showImageDetails);
...
searchElement.addEventListener('keyup', (event) => {
    ...
    cleanView();
    // const moviesFiltered = searchingWithFor(inputText);
    const moviesFiltered = searchingWithFilter(inputText);
    moviesFiltered.forEach(renderCardContact);
});

const searchingWithFor = (searchingText) => {
    let moviesFiltered = [];
    for (let i=0; i < movies.length; i++) {
        const contact = movies[i];
        const name = contact.text;
        if ((name.toLocaleLowerCase()).includes(searchingText)) {
            moviesFiltered.push(contact);
        }
    }
...
const searchingWithFilter = (searchingText) => {
    const moviesFiltered = movies.filter(contact => {
        const name = contact.text;
